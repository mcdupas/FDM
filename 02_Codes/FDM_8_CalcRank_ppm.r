...
Sim_ppplist = simulate.ppm(fit_kppm, nsim = P_ValNum, window=win_border)
...
# compute the summary statistics on each simulated pattern
#Sim_ppplist = simulate(fit_kppm, nsim = P_ValNum)

#registerDoParallel(cores=P_Cores)  # choose the number of of cores to use while paralleling
#Sim_Linhom = foreach(j=1:P_ValNum, .combine=cbind) %dopar% { 
#  spatstat::Linhom(spatstat::as.ppp(Sim_ppplist[[j]]), correction = "none")$un }
...
p<-barplot(data2[,2],names.arg=data2[,1],las=2, ylab="Log10(Importance Covariate)")
