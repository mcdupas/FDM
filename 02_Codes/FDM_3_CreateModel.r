...
FD = st_transform(FD, crs=CRS_latlon)    
...
FD_sp_small = FD_sp[FD_sp$Stock <= P_MinStock,]
FD_sp_large = FD_sp[FD_sp$Stock >= P_MinStock,]
...
P_WindowFolder = paste(P_ProcessingFolder,"/win_border.shp",sep="")
...
#Save model to disk
kppmPath = paste(P_ProcessingFolder,"/","model_Access.kppm", sep="")
...
write.csv(df,paste(P_ProcessingFolder,"/","importance_covariates_Access.csv", sep=""), row.names = FALSE)
